...
- (NSString *)composeURLForCommand:(NSString *)command withParams:(NSDictionary *)params returnMode:(TCReturnMode)returnMode
{
	...
	if (returnMode == TCPlist)
	{
		...
		[tempDict setValue:@"1" forKey:@"asPlist"];
...
{
	[self doServerCommand:command withParams:params callingMethod:methodSelector ofObject:receivingObject withReturnMode:TCRaw];
}
...
{
	[self doServerCommand:command withParams:params callingMethod:methodSelector ofObject:receivingObject withReturnMode:TCPlist];
}

- (void)doServerCommand:(NSString *)command withParams:(NSDictionary *)params callingMethod:(SEL)methodSelector ofObject:(id)receivingObject withReturnMode:(TCReturnMode)returnMode
{
	urlString = [self composeURLForCommand:command withParams:params returnMode:returnMode];
	...
	if (theConnection)
	{
		...
		[properties setObject:[[NSDictionary alloc] initWithObjectsAndKeys:responseInvocation, @"invocation", [NSNumber numberWithInt:returnMode], @"returnMode", nil] forKey:connectionID];
...
- (NSDictionary *)doBlockingCommand:(NSString *)command withParams:(NSDictionary *)params
{
	urlString = [self composeURLForCommand:command withParams:params returnMode:TCPlist];
	...
	NSData *response = [NSURLConnection sendSynchronousRequest:theRequest returningResponse:&theResponse error:&errorResult];
	NSString *errorString;
	
	return [NSPropertyListSerialization propertyListFromData:response mutabilityOption:NSPropertyListImmutable format:nil errorDescription:&errorString];
}
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection
{
	...
	int returnMode = [[[properties objectForKey:cID] objectForKey:@"returnMode"] intValue];
	if (returnMode == TCRaw)
	{
	...
	else if (returnMode == TCPlist)
	{
		NSString *errorString = nil;
		theUserInfo = [NSPropertyListSerialization propertyListFromData:[responses objectForKey:cID] mutabilityOption:NSPropertyListImmutable format:nil errorDescription:&errorString];
	}
