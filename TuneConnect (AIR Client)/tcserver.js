...
TCServer.prototype = {
	...
	completeConnection: function(response) {
		...
		if (response.version >= 1) {
			...
			air.trace('Connected to server at '+this.serverAddress());
			this.connected = true;
	...
	handleAuthKey: function(response) {
		if (response.authKey != false) {
			...
			air.trace('Could not secure an auth key.');
...
TCServer.prototype.doCommand = function(command, params, onComplete, asJSON) {
	...
	requireAuthKey = (this.requiresPassword && this.authKey != '');
	...
	air.trace('Performing command: '+command);
	//new Ajax.Request(this.composeURIForCommand(command, null, false), options)
	var request = new air.URLRequest(this.composeURIForCommand(command, params, requireAuthKey));
	request.userAgent = 'TuneConnect AIR/0.1';
	var loader = new air.URLLoader();
	loader.addEventListener(air.Event.COMPLETE, this.commandHandler.bind({'TCSettings':options}));
	loader.load(request);
}
...
TCServer.prototype.commandHandler = function(event) {
	var loader = event.target;
	air.trace('Response received...');
